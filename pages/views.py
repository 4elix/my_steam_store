...
from .models import Categories, Games, Favorite, Review, Rating, ViewsGame, CommentLike
...
def favorite_logic(request, pk_game):
    if request.user.is_authenticated:
        user = request.user
...
def comment_like_logic(request, comment_id):
    if request.user.is_authenticated:
        user = request.user
        status = CommentLike.objects.filter(user=user, comment_id=comment_id).exists()
